 ...
 *******************************************************************
 Module name: wirTwilightFlat
 Version: 1.0
 Description: Module to create a twilight flat
 ...
 * wirTwilightFlat
 * \author Doug Teeple
 * \brief Module to create a twilight flat.
 * \arg argc
 * \arg argv
 * \note --output=...
 * \note --input=...
...
int main(int argc, char *argv[])
{
	...
	try {
		if (flat_count == 0) {
			throw operaException("wirTwilightFlat: ", operaErrorNoInput, __FILE__, __FUNCTION__, __LINE__);	
		}
		if (name_twilightflat.empty()) {
			throw operaException("wirTwilightFlat: ", operaErrorNoInput, __FILE__, __FUNCTION__, __LINE__);	
		}
		...
		if (verbose) {
			for (unsigned i=0; i<flat_count; i++) {
				cerr << "wirTwilightFlat: input flat[" << i << "] = " << name_flats[i] << endl; 
			}
			cerr << "wirTwilightFlat: output image = " << name_twilightflat << endl; 
			cerr << "wirTwilightFlat: weight map image = " << name_weightmap << endl; 
			
            if (plot) {
                cerr << "wirTwilightFlat: plotfilename = " << plotfilename << endl;
                cerr << "wirTwilightFlat: datafilename = " << datafilename << endl;
                cerr << "wirTwilightFlat: scriptfilename = " << scriptfilename << endl; 
            }            
		}
		...
		if (verbose) {
	        cout << "wirTwilightFlat: Reading files in to memory " << endl;
		}
		operaWIRCamImage *flats[MAXFLATS];
		for (unsigned i=0; i<flat_count; i++) {
			flats[i] =  new operaWIRCamImage(name_flats[i], tfloat, READONLY);
		}
		operaWIRCamImage out(name_twilightflat, WIRCAM_NAXIS1, WIRCAM_NAXIS2, 1, WIRCAM_EXTENSIONS, tfloat);
		...
		// Create twilight flat
		out.masterFlatParallel(flats, &weightmap, flat_count); 

        // Save the twilight flat image
		if (verbose) {
	        cout << "wirTwilightFlat: Saving output" << name_twilightflat << endl;
		}
        ...
        if (fdata != NULL) {
            ...
            if (!plotfilename.empty() && !scriptfilename.empty()) {
                GenerateTwilightFlatPlot(scriptfilename.c_str(),plotfilename.c_str(),datafilename.c_str(), false);
            }
        }                
	}
	catch (operaException e) {
		cerr << "wirTwilightFlat: " << e.getFormattedMessage() << endl;
		return EXIT_FAILURE;
	}
	catch (...) {
		cerr << "wirTwilightFlat: " << operaStrError(errno) << endl;
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}
...
void GenerateTwilightFlatPlot(const char *gnuScriptFileName, const char *outputPlotEPSFileName,const char *dataFileName, bool display)
{
    FILE *fgnu;
