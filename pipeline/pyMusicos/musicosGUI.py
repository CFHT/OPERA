...
###########################################
configfilepath = os.path.realpath(__file__).strip('musicosGUI.py') + 'config.musicospipeline'
if os.path.isfile(configfilepath) :
    print_message("CONFIG FILE: " + configfilepath)
else :
    print_ERRORmessage( "ERROR: Non-existent CONFIG file \'" + configfilepath + "\'")
###########################################

###########################################
########### PLOT FLAT CALIBRATION #########
def plot(filename, x, y,line_style, title,x_label,y_label,line_label,y_range,skip_lines):
    open_file = open(filename, "r")
    if skip_lines == False:
        lines = open_file.readlines()[19:]
    elif skip_lines == True:
    ...
    for line in lines:
        column = line.split()
        axis_x.append(float(column[x]))
        axis_y.append(float(column[y]))

    x_vector = np.array(axis_x)
    y_vector = np.array(axis_y)
    ...
    plt.xlabel(x_label)
    plt.ylabel(y_label)
    plt.plot(x_vector,y_vector,line_style, color = "k", label= line_label)
    plt.legend()
    plt.show() 
###########################################
...
############ CONFIG FILE ##################   
    def OnPressEnterConfigPathfield(self,event):
        ...
        if os.path.isfile(self.filepathVariable.get()) :    
            ...
            self.bottombarlabelVariable2.set("MUSICOS Pipeline home dir:" + self.operaDirVariable.get())
...
######## CHOOSE MODE FOR PLOT #############  
        self.sidetextpd = Tkinter.StringVar()        
        sidetextlabelpd = Tkinter.Label(self,textvariable=self.sidetextpd, anchor="w",fg="lightgray",bg="black")
        sidetextlabelpd.grid(column=0,row=9,columnspan=1)        
        self.sidetextpd.set("CHOOSE MODE FOR PLOTS:")
        self.mode = Tkinter.StringVar()
        self.mR = Tkinter.Radiobutton(self, text="RED", variable=self.mode, value="RED")
        self.mR.grid(column=1,row=9,columnspan=1) 	
	
        self.mB = Tkinter.Radiobutton(self,text=u"BLUE", variable=self.mode, value="BLUE")
        self.mB.grid(column=2,row=9,columnspan=1)
...
############# FINAL PLOTS #################  
        ...
        self.NormButton.grid(column=1,row=11,columnspan=1)
        ...
        self.ONEDButton.grid(column=2,row=11,columnspan=1)
#############################################
...
############### RUN MUSICOS PIPELINE ######       
        # Below is the button to run Musicos PIPELINE
...
##### RUN MUSICOS PIPELINE SIMULATION #####       
        # Below is the button to run Musicos PIPELINE
...
################# CONSOLE #################        
        ...
        self.bottombarlabelVariable1.set("MUSICOS Pipeline 1.0 Console by E. Martioli, R.Grossova")      
...
########### OPERA BUTTON ##################   
    def OnRunOperaButtonClick(self):
        ...
        if os.path.exists(self.ddentryVariable.get()) and os.path.exists(self.pdentryVariable.get()) :  
            self.bottombarlabelVariable1.set("Running MUSICOS Pipeline for " + self.ddentryVariable.get() + self.nightentryVariable.get() + "/ ...")
            self.bottombarlabelVariable2.set("")                        
            self.update()     
            print_infomessage("Running OES Pipeline for NIGHT: " + self.nightentryVariable.get())
            # command line to run pipeline script             
            commandline = self.operaDirVariable.get() + '/pipeline/pyMusicos/operaMusicos.py' \
...
########### OPERA SIMULATION BUTTON #######
    def OnRunOperaSimButtonClick(self):
        ...
        # test if directory exists if not it will ask for it.
        if os.path.exists(self.ddentryVariable.get()) and os.path.exists(self.pdentryVariable.get()) :  
            self.bottombarlabelVariable1.set("Running MUSICOS Pipeline Simulation for " + self.ddentryVariable.get() + self.nightentryVariable.get() + "/ ...")
            self.bottombarlabelVariable2.set("")                        
            self.update()    
            print_infomessage("Running MUSICOS Pipeline Simulation for NIGHT: " + self.nightentryVariable.get())
...
############# GEOMETRY PLOTS ############# 
    def OnGeomPlotsButtonClick(self):
        ...
        if os.path.exists(self.ddentryVariable.get()) and os.path.exists(self.pdentryVariable.get()) :    
            ...
            spcprod = self.pdentryVariable.get() + self.nightentryVariable.get() + '/' + self.nightentryVariable.get() + "_" + self.mode.get() + "_" + self.readoutVariable.get() + "_spcplot.eps" 
            geomprod = self.pdentryVariable.get() + self.nightentryVariable.get() + '/' + self.nightentryVariable.get() + "_" + self.mode.get() + "_" + self.readoutVariable.get() + "_geomplot.eps" 
...
############# APERTURE PLOTS #############      
    def OnAperPlotsButtonClick(self):
        ...
        if os.path.exists(self.ddentryVariable.get()) and os.path.exists(self.pdentryVariable.get()) :    
            ...
            profprod = self.pdentryVariable.get() + self.nightentryVariable.get() + '/' + self.nightentryVariable.get() + "_" + self.mode.get() + "_" + self.readoutVariable.get() + "_profplot.eps"
            aperprod = self.pdentryVariable.get() + self.nightentryVariable.get() + '/' + self.nightentryVariable.get() + "_" + self.mode.get() + "_" + self.readoutVariable.get() + "_aperplot.eps"
            tiltprod = self.pdentryVariable.get() + self.nightentryVariable.get() + '/' + self.nightentryVariable.get() + "_" + self.mode.get() + "_" + self.readoutVariable.get() + "_tiltplot.eps"
...
########## WAVELENGHT CALIB PLOTS ############# 
    def OnCalibPlotsButtonClick(self):
        ...
        if os.path.exists(self.ddentryVariable.get()) and os.path.exists(self.pdentryVariable.get()) :    
            ...
            # command line below vizualizes calibration result 
            wavespecprod = self.pdentryVariable.get() + self.nightentryVariable.get() + '/' + self.nightentryVariable.get() + "_" + self.mode.get() + "_" + self.readoutVariable.get() + "_wavespecplot.eps" 
            waveordsprod = self.pdentryVariable.get() + self.nightentryVariable.get() + '/' + self.nightentryVariable.get() + "_" + self.mode.get() + "_" + self.readoutVariable.get() + "_waveordsplot.eps"
...
############ FLAT FIELD CALIB PLOTS ###########
    def OnFcalPlotsButtonClick(self):
        ...
        if os.path.exists(self.ddentryVariable.get()) and os.path.exists(self.pdentryVariable.get()) :    
            ...
            print_infomessage("Checking the wavelength solution products for night: " + self.nightentryVariable.get())
            # command line below vizualizes calibration result 
            gzffcal = self.pdentryVariable.get() + self.nightentryVariable.get() + '/' + self.nightentryVariable.get() + "_" + self.mode.get() + "_" + self.readoutVariable.get() + "_flat.fcal.gz"            
            ffile = self.pdentryVariable.get() + self.nightentryVariable.get() + '/' + self.nightentryVariable.get() + "_" + self.mode.get() + "_" + self.readoutVariable.get() + "_flat.fcal"
            name = ffile.split('/')[-1]
            ...
            if os.path.exists(gzffcal):
                ...
                plot(ffile,5,6,'-',"Flat flux calibration", "Wavelength (nm)", "Order number + Normalized flux", name,y_range=None,skip_lines=True)
    ...
    def OnRawPlotButtonClick(self):
        ...
        if os.path.exists(self.ddentryVariable.get()) and os.path.exists(self.pdentryVariable.get()) :    
            ...
            if not os.listdir(FinalProdDir) == []:
                ...
                commandline = self.operaDirVariable.get() + '/bin/operaQueryImageInfo --directory=' + nightdir + ' -e "DATE-OBS INSTMODE OBSTYPE IMAGE"' 
                print_message( "FILENAME/DATE/MODE/TYPE/OBJECT_NAME")    
                os.system(commandline)
                print "-----------------------------------------------------------------"
                ...
                if chooseFile:
                    ...
                    if openFile != None:
                        data = openFile.read()                
                        openFile.close()
                    plot(chooseFile,4,8,'-', "Raw spectrum reduced by OPERA", "Wavelength (nm)", "Raw flux", name,y_range=None,skip_lines=True)
            ...
            if not os.listdir(FinalProdDir) == []:
                ...
                if chooseFile:
                    ...
                    if openFile != None:
                        data = openFile.read()                
                        openFile.close()
                    plot(chooseFile,4,12,'-', "Calibrated spectrum reduced by OPERA", "Wavelenght (nm)", "Raw flux / Flat-field (arbitrary units)", name,y_range=None, skip_lines=True)
                    #os.system("cd %s" % FinalProdDir)
            ...
            if not os.listdir(FinalProdDir) == []:
                ...
                if chooseFile:
                    ...
                    if openFile != None:
                        data = openFile.read()                
                        openFile.close()
                    plot(chooseFile,4,10,'-', "Normalized spectrum reduced by OPERA", "Wavelength (nm)", "Normalized flux", name,[0.0,1.5],skip_lines=True)
            ...
            if not os.listdir(ONEdProdDir) == []:
                ...
                if chooseFile:
                    ...
                    if openFile != None:
                        data = openFile.read()                
                        openFile.close()
                    plot(chooseFile,0,1,'-', "1D spectrum reduced by OPERA", "Wavelength (nm)", "Normalized flux", name,[0.0,1.5], skip_lines=False)
...
###############################################
    def Onds9ButtonClick(self):
		...
		print file   
            ...
            chooseFile = tkFileDialog.askopenfilename(title='Choose a file', defaultextension='.fits', filetypes =[("FITS extension",".fits"),("all files",".*")],initialdir=self.dataNightVariable.get()) 
            if chooseFile: 
...
if __name__ == "__main__":
    ...
    app.title('MUSICOS Pipeline v. 1.0')
    app.mainloop()
