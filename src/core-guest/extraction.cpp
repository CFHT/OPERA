...
int main(int argc, char *argv[])
{
	...
	while((opt = getopt_long(argc, argv, "i:g:s:G:o:b:l:r:N:O:X:m:A:W:w:pvdth", 
							 longopts, NULL))  != -1)
	{
		switch(opt) 
		{
	...
	try {
		...
		if (inputImageName.empty()) {
			throw operaException("extraction: ", operaErrorNoInput, __FILE__, __FUNCTION__, __LINE__);	
		}
		// we need an output...
		if (outputSpectraFile.empty()) {
			throw operaException("extraction: ", operaErrorNoOutput, __FILE__, __FUNCTION__, __LINE__);	
		}
		...
		if (verbose) {
			cout << "extraction: input image = " << inputImageName << endl; 
			cout << "extraction: input geometry file = " << inputGeometryName << endl; 			
			cout << "extraction: input wavelength file = " << inputWavelengthName << endl; 			
			cout << "extraction: input gain file = " << inputgain << endl;            
			cout << "extraction: output spectra file = " << outputSpectraFile << endl;
			cout << "extraction: badpixelmask = " << badpixelmask << endl; 				
			cout << "extraction: X-aperture = " << aperture << endl; 
			cout << "extraction: Y-height = " << spectralElementHeight << endl; 				
		}
		...
		if (verbose)
			cout << "extraction: gain=" << gain << " noise=" << noise << endl;
		...
		for (unsigned order=minorder; order<=maxorder; order++) {
			...
			if (spectralOrder->gethasGeometry()) {
				if ((!inputWavelengthName.empty() && spectralOrder->gethasWavelength()) || inputWavelengthName.empty()) {
					...
					spectralOrder->extractRawSum(inputImage, gain, noise);
					if (!inputWavelengthName.empty()) {
		...
		cerr << "extraction: " << e.getFormattedMessage() << endl;
		return EXIT_FAILURE;
	}
	catch (...) {
		cerr << "extraction: " << operaStrError(errno) << endl;
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}
