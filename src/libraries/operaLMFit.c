...
/*!
 * \brief  Levenberg Marquardt Least Squares Fitting
 * \file operaLMFit.c
 * \ingroup libraries
*/
/*
					  ...
					  double *fvec, int *info )
{
    ...
    for ( i = 0; i < m_dat; i++ )
		...
		((lmcurve_data_struct*)data)->function(((lmcurve_data_struct*)data)->x[i], par, n_par);
			  ...
			  int printflags, const void *data )
{
	 ...
	 *
	 *        void evaluate( int n_par, double* par, int m_dat, void *data,
	 *                       double* fvec, int *info )
	 *        {
	 *           // for ( i=0; i<m_dat; ++i )
    ...
    *info = 0;
    ...
    do {
		...
		do {
			...
			*info = 0;
			(*evaluate) (n, wa2, m, data, wa4, info);
            ++(*nfev);
            if( printout )
